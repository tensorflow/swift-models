...
public struct UNetSkipConnectionInnermost: Layer {
    ...
    public init(
        ...
        self.downConv = Conv2D(
            filterShape: (4, 4, inChannels, innerChannels),
            strides: (2, 2),
            padding: .same,
            filterInitializer: {
                Tensor<Float>(
        ...
        self.upConv = TransposedConv2D(
            filterShape: (4, 4, innerChannels, outChannels),
            strides: (2, 2),
            padding: .same,
            filterInitializer: {
                Tensor<Float>(
...
public struct UNetSkipConnection<Sublayer: Layer>: Layer
where
    ...
    Sublayer.Output == Tensor<Float>
{
    ...
    public init(
        ...
        self.upConv = TransposedConv2D(
            filterShape: (4, 4, outChannels, innerChannels * 2),
            strides: (2, 2),
            padding: .same,
            filterInitializer: {
                Tensor<Float>(
...
public struct UNetSkipConnectionOutermost<Sublayer: Layer>: Layer
where
    Sublayer.TangentVector.VectorSpaceScalar == Float, Sublayer.Input == Tensor<Float>,
    Sublayer.Output == Tensor<Float>
{
