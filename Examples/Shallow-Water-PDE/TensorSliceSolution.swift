...
struct TensorSliceSolution: ShallowWaterEquationSolution {
    ...
    func evolved() -> TensorSliceSolution {
        var Δu0 = Δ(u0)
        var Δu1 = Δ(u1)
        Δu0 = Δu0.padded(forSizes: [(before: 1, after: 1),
                                    (before: 1, after: 1)], with: 0.0)
        Δu1 = Δu1.padded(forSizes: [(before: 1, after: 1),
                                    (before: 1, after: 1)], with: 0.0)
...
fileprivate extension TensorShape {
    var tensor: Tensor<Int32> { Tensor<Int32>(dimensions.map(Int32.init)) }

    static func - (lhs: TensorShape, rhs: Int) -> TensorShape {
