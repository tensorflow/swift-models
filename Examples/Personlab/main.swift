...
// Pre-processing
var frame = imread(config.testImagePath)
...
var normalizedImagesTensorBGR = image.tensor * (2.0 / 255.0) - 1.0
let normalizedImagesTensorRGB = _Raw.reverse(normalizedImagesTensorBGR, dims: [false, false, false, true])

// Run pose estimator
let startTime = Date()
var convnetResults = personlabHeads(backbone(normalizedImagesTensorRGB))
