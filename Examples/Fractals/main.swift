...
extension FractalCommand {
    struct JuliaSubcommand: ParsableCommand {
        ...
        func run() throws {
            ...
            let divergenceGrid = juliaSet(iterations: iterations ?? 200, constant: constant ?? ComplexConstant(real: -0.8, imaginary: 0.156), tolerance: tolerance ?? 4.0, region: region ?? ComplexRegion(realMinimum: -1.7, realMaximum: 1.7, imaginaryMinimum: -1.7, imaginaryMaximum: 1.7), imageSize: imageSize ?? ImageSize(width: 1030, height: 1030), device: device)
            
            do {
                try saveFractalImage(divergenceGrid, iterations: iterations ?? 200, fileName: outputFile ?? "julia")
            } catch {
...
extension FractalCommand {
    struct MandelbrotSubcommand: ParsableCommand {
        ...
        func run() throws {
            ...
            let divergenceGrid = mandelbrotSet(iterations: iterations ?? 200, tolerance: tolerance ?? 4.0, region: region ?? ComplexRegion(realMinimum: -2.0, realMaximum: 1.0, imaginaryMinimum: -1.3, imaginaryMaximum: 1.3), imageSize: imageSize ?? ImageSize(width: 1030, height: 1030), device: device)
            
            do {
                try saveFractalImage(divergenceGrid, iterations: iterations ?? 200, fileName: outputFile ?? "mandelbrot")
            } catch {
                print("Error saving fractal image: \(error)")
            }
        }
