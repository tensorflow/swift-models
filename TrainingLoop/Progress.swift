...
public func trainingProgress<L: TrainingLoopProtocol>(_ loop: inout L, event: TrainingLoopEvent) throws {
    switch event {
    case .epochStart:
    ...
    case .batchEnd:
        ...
        let progressSteps = Int(round(Float(progressBarLength) * epochProgress))
        ...
        let progressBar = "[\(leading)\(separator)\(trailing)]"
        print("\u{1B}[1A\u{1B}[K\(batchIndex + 1)/\(batchCount) \(progressBar)")
